...
import {
	DemoApprovalModal,
	DemoEventModal,
} from "./components/OnboardingDemoModals";
import {
	CalendarSkeleton,
...
function AppContent() {
	...
	const approvalItem =
		approvalIndex >= 0 ? pendingItems[approvalIndex] : null;
	...
	// Check if demo modals should be shown based on current tour step
		...
		return events.map((event) => {
			...
			return {
				...
				leftSection: (
					<StatusIcon
						size={20}
						color={cls?.color || unassignedColor}
					/>
				),
	...
	// Create tour steps with step tracking via content function
	const tourStepsWithTracking = tourSteps.map((step, index) => ({
		...step,
		content: (controller) => {
			// Update step tracking when this step's content renders
			if (onboardingStep !== index) {
				// Use setTimeout to avoid state update during render
				setTimeout(() => setOnboardingStep(index), 0);
			}
			// Add dynamic class to body for CSS targeting
			if (index === 4 || index === 5) {
				// Demo modal steps
				setTimeout(() => {
	...
	return (
		...
		>
			...
			>
				<AppShell.Header>
					<Group h="100%" px="md" justify="space-between">
						...
						<Group>
							<Group
								gap="xs"
								data-onboarding-tour-id="header-utilities"
							>
				...
				{/* Demo modals for onboarding tour */}
